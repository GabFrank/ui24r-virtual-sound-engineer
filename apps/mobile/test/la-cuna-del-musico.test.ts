...
function volcado(pares: Readonly<Record<string, number>>): ReadonlyMap<string, { valor: number }> {
...
test('su propio instrumento va primero aunque esté cerrado y otro mande más', () => {
  const filas = loQueLlegaALaCuna(1, 'ana', CANALES, ASIGNACIONES, volcado({
...
test('si de la cuña se leyó algo, el ajeno que falta SÍ se muestra: es una anomalía', () => {
  ...
  const filas = loQueLlegaALaCuna(1, 'ana', CANALES, ASIGNACIONES, volcado({
    [envio(1)]: 0.5,
    // el 2, el 3 y el 4 no están: el 2 es suyo y se muestra siempre; el 3 es
    // ajeno y falta con la cuña publicándose, así que hay algo raro y se ve.
  }));
...
test('entre los ajenos manda el que más manda, y lo que no se sabe va al final', () => {
  ...
  const filas = loQueLlegaALaCuna(1, 'ana', CANALES, ajenos, volcado({
    [envio(1)]: 0.4,
    [envio(2)]: 0.9,
...
test('un músico sin identificador no se queda con los canales sin asignar', () => {
  ...
  const filas = loQueLlegaALaCuna(1, null, CANALES, sinDuenio, volcado({
...
test('sin consola conectada se ve igual el instrumento del músico', () => {
  ...
  const filas = loQueLlegaALaCuna(1, 'ana', [], ASIGNACIONES, volcado({}));
...
test('un canal asignado que la consola no publica se nombra con su instrumento', () => {
  ...
  const filas = loQueLlegaALaCuna(1, 'ana', soloUno, ASIGNACIONES, volcado({
...
test('un auxiliar que no existe no arma ninguna fila, en vez de armarlas mal', () => {
  assert.deepEqual(loQueLlegaALaCuna(11, 'ana', CANALES, ASIGNACIONES, volcado({})), []);
});
