...
| `busDeAnalisis` de `clasificarRuta` (INV-008) | el motor de seguridad | Cuál auxiliar es el bus de análisis lo dice SPK-P0.5. **Esta fila decía «sin ese dato ningún envío es escribible — el lado seguro», y ADR-028 lo dio vuelta el 2026-09-12**: ahora el envío de monitor sí se escribe, así que no pasar el dato hace que los 24 envíos hacia el bus de análisis se traten como envíos de monitor escribibles, con los topes y el techo de un monitor y no los de un bus de medición. El motor llama a `clasificarRuta` sin opciones y **nadie en producción pasa el dato**. Hoy no tiene consecuencia porque ningún camino de la aplicación propone un cambio de envío; el día que lo proponga, sí. Lo encontró una auditoría de fidelidad. |
...
| `SafetyService.crearEjecutor` (INV-034) | quien aplique una transacción | **Ya no está vacío**: desde ADR-026 la pantalla de ganancia aplica por acá, y el servicio de bajar un envío a monitor también lo usa, aunque nadie lo llame todavía. La fila decía «la aplicación no escribe en la consola» y eso dejó de ser cierto el 2026-09-13. |
...
| INV-004 | Delta máximo por transacción: fader ±3 dB; gain ±3 dB; EQ ±3 dB (salida) / ±4 dB (entrada), Q ≥ 0,7 en salidas; HPF ≤ 1 octava; delay ≤ 5 ms; master ±1 dB y nunca por encima del máximo previo de la sesión. **Tope acumulado por parámetro y sesión** respecto a la **referencia vigente** —el valor inicial de la sesión, o el nivel de trabajo desde que alguien lo estableció, ver INV-010—: fader ±6, gain ±6, EQ ±6 por banda, HPF ≤ 2 octavas, delay ≤ 10 ms. Una nueva transacción sobre el mismo parámetro solo es elegible si existe una `Measurement` posterior a la anterior. **La cláusula «Q ≥ 0,7 en salidas» no se puede cumplir en esta consola, y hay que decirlo: su ecualizador de salida es un GRÁFICO de 31 bandas por lado (`m.eq.peak.l.0`…`.30`), sin factor de calidad.** El guardia del motor es `if (c.q !== undefined && …)`, así que sobre un gráfico no se dispara nunca — la forma nueva de «la constante que nadie consulta». El peligro que la cláusula quiere evitar, un realce estrecho sobre el sistema, en un gráfico lo acota `REALCE_MAXIMO_SALA_DB`, que sí se consulta. Se conserva para un paramétrico de salida, que la Ui24R no tiene.** **Y el tope numérico comparaba decibeles contra crudo, así que no se disparaba nunca.** El motor calculaba el delta como `valorPropuesto - valorEsperado` —en crudo— y lo comparaba contra los topes de esta fila, que están en dB. El crudo va de 0 a 1 y el tope de la ganancia es 3: **medido, el mayor salto que dejaba pasar era de 61,9 dB**, el recorrido entero del previo, de −6,0 a +55,9. El campo `unidad` de `CambioPropuesto` existía desde el principio y no lo leía nadie: se copiaba al diario, o sea que el diario registraba «dB» al lado de un número que no eran decibeles. **Y la suite estaba verde**, porque todos los tests pasaban valores que sí eran decibeles y el único llamador de producción pasaba crudo. Desde el 2026-09-11 el cambio lleva `magnitudPropuesta`/`magnitudEsperada` **obligatorias** en la unidad declarada, el crudo sigue yendo al cable, y hay una guarda que rechaza que los dos salgan del mismo sitio. **Desde el 2026-09-17 el tope acumulado admite una suspensión, y viene con su propia guarda.** ADR-034 necesita que el presupuesto por sesión no rija mientras el parámetro todavía no tiene un nivel del que desviarse —el caso del envío a monitor, ver INV-010—, y la suspensión **sólo se concede si ese tipo declara `techoAbsoluto`**: cambiar un freno por otro exige que el otro exista, y un presupuesto suspendido sin techo dejaría el parámetro sin ningún tope sobre el total. Se pide con `nivelEstablecido: false`, que **sin declarar significa «establecido»** para que la excepción no se obtenga por omisión. Y el techo se comprueba contra la magnitud resultante, que quien propone tiene que declarar: si el tipo tiene techo y esa magnitud no viene, el cambio se rechaza en vez de pasar sin comprobar. **Y un número que no es un número pasaba esta invariante entera, comprobado y tapado el 2026-09-17.** Toda comparación con `NaN` da `false`, así que `Math.abs(NaN) > tope` es `false`, y lo mismo el acumulado y el techo: un cambio que declarara `NaN` en su magnitud quedaba aprobado por los tres. Y no se quedaba ahí — `verificarAtadura`, la guarda que comprueba que el motor juzgue el mismo número que va al cable, devolvía `atada: true` por el mismo motivo, así que el `NaN` pasaba **con cualquier crudo**. No estaba expuesto: los dos servicios de producción calculan magnitudes finitas, y el motor es justamente la pieza que no puede depender de que quien lo llama haga las cosas bien. Se tapó **sólo `NaN`**: un delta infinito lo sigue rechazando el tope por transacción, que es lo correcto mientras nadie sepa proponer un salto desde el silencio. **Y los topes de esta fila acotan un movimiento, que tiene dos extremos, y hasta el 2026-09-17 se ataba uno.** `verificarAtadura` ataba `magnitudPropuesta` al crudo que va al cable desde el 2026-09-13; `magnitudEsperada` —el otro sumando de la resta que estos topes comparan— **no estaba atada a nada**. Una auditoría adversarial lo midió: **un salto de 31 dB en la cuña de un músico pasa el tope de 2 dB por paso declarando que venía de un decibel más abajo**, con el destino honesto y el punto de partida inventado. Desde el 2026-09-17 lo ata `verificarAtaduraDelOrigen`, con la misma ley, la misma holgura del 1 % del recorrido y el mismo tratamiento de `SIN_LEY_VERIFICADA`, y rechaza con código propio, `ORIGEN_NO_ATADO`. **Dónde cierra la cadena, que una auditoría de fidelidad tuvo que corregir el mismo día:** la primera redacción de esta fila decía «la cadena queda entera: estado confirmado → `valorEsperado` → `magnitudEsperada` → el movimiento → el tope», y presentaba como propiedad del motor algo que es del adaptador. **El motor no lee la consola**: compara dos números que declara el mismo llamador, y mentirlos los dos de forma coherente sigue dando `permitido: true`, medido. Quien ata `valorEsperado` al estado confirmado es `coincideConEsperado` (INV-011), **después** del veredicto y antes de enviar, devolviendo `CONFLICT`. La garantía que se sostiene hay que enunciarla sobre el cable: **ninguna escritura sale con el movimiento mal medido** —o el origen declarado es el de verdad, y entonces esta guarda comprueba sus decibeles, o no lo es, y la escritura no sale—. Lo que este paso cerró es el hueco entre las dos: crudo de partida verdadero, para que el adaptador lo acepte, y decibeles de partida falsos, para que el motor mida mal; eso pasaba entero. Tampoco es exacto decir que se rechaza «antes de tocar nada»: los dos servicios guardan una instantánea en la consola **antes** de llamar al ejecutor, así que el punto de retorno ya se escribió. **Importa más desde ADR-034 que cuando se encontró**: con el presupuesto acumulado suspendido mientras la cuña no tiene nivel establecido, el tope por paso es el **único** freno sobre la brusquedad de la rampa. **Y ese freno hereda la holgura, así que no son 2 dB: son 2,84.** Atar es comparar con tolerancia, y la tolerancia es el 1 % del recorrido —0,4214 dB en el envío a monitor—, mientras el adaptador compara el crudo con 1e-9. **La holgura se cobra en los dos extremos**: el origen declarado un poco más arriba y el destino un poco más abajo suman, y medido con el motor un paso real de **2,84 dB pasa y uno de 2,85 no**. Son 0,84 dB de juego, un 42 % sobre el tope. Una primera redacción de esta fila dijo 2,42 porque midió sesgando un solo extremo, y lo corrigió una auditoría adversarial el mismo día. **Y la comparación honesta es contra lo que había antes, que no era 0,84 dB sino ilimitado**: el origen no estaba atado a nada. Esto no deja el tope exacto, lo deja acotado. Es la misma holgura del ítem 2 de esa auditoría —el techo de nominal— pesando acá más que allá: allá acota dónde se termina, acá cuán brusco es cada paso. **Y una auditoría adversarial encontró, en este mismo commit, que la guarda se caía abierta por el otro operando.** `atar` exigía que la MAGNITUD fuera finita y **nunca miraba el CRUDO**: con `valorEsperado` en `NaN`, `fromRaw` da `NaN`, la comparación de la holgura es `false` y contestaba `atada: true`. Medido de punta a punta con el ejecutor real: declarando un movimiento de 1 dB, **la cuña se movió 31 dB, la transacción quedó `APLICADA` y la escritura salió al cable** — o sea el mismo salto que la atadura del origen existe para cerrar, entrando por el operando de al lado, y el mismo `NaN` que INV-004 ya había tapado dos veces. Se cerró con `CRUDO_NO_NUMERICO`, en los dos extremos, y con su mutante comprobado. **Lo que no se cerró acá porque es de otra invariante y otro paquete, y queda como tarea:** `coincideConEsperado` (INV-011) tiene hoy exactamente la misma forma —`Math.abs(e.valor - esperado) > tolerancia` con `NaN` da `false`— y contesta que coincide. No está expuesto **porque nada llega al adaptador sin pasar por el motor**, y ésa es toda la protección que tiene. **Y lo que sigue sin atarse, con todas las letras:** sólo quedan atadas las rutas cuya ley está medida contra el aparato. La **ganancia del previo no lo está** —no figura en `rutasProbadas()`—, así que para el único parámetro que la aplicación mueve hoy de punta a punta el motor sigue juzgando los dos extremos que el llamador declara. Eso no mejora solo: mejora cuando alguien mida esa ley. **Un efecto secundario que es un cambio de comportamiento, no de nombre, y la primera redacción lo decía mal:** el silencio de una cuña es el crudo 0 y por la ley medida son −∞ dB, así que desde ahí **no hay ningún valor que la guarda acepte** —ni un número finito, que no coincide, ni −∞, que no es número—. Esta fila afirmó que era «el mismo veredicto que ya daba el tope por paso»; la auditoría lo midió y no lo es: un paso declarado como chico desde el crudo 0 —−32 a −31— **pasaba** y ahora se rechaza, y la prueba está en que hubo que mover el test `ADR-028: con todo abajo al empezar` al tramo medido para que siguiera pasando. Que ahora se rechace es lo correcto, porque esa declaración era falsa —el crudo 0 no son −32 dB— y lo que se escribía era un salto desde el silencio disfrazado de pasito. **Y le cambia el trabajo a la pieza que sigue de ADR-034**: antes, proponer el primer paso desde el silencio necesitaba una excepción al tope por paso; ahora necesita además un caso con nombre propio dentro de `atar`, porque el destino que el usuario eligió —arrancar en −32,14 dB— ya no se puede ni expresar desde el crudo 0. Falla cerrado, que es lo correcto mientras nadie sepa proponerlo, y es un borde que hay que abrir a propósito. | Unit: 3 transacciones consecutivas de +3 dB sin Measurement intermedia → la tercera rechazada con `CUMULATIVE_CAP`; un tipo sin `techoAbsoluto` no consigue la suspensión aunque la pida; HIL: asserts sobre comandos. | MVP0 (recomendaciones), MVP4a (writes) |
...
| INV-010 | **Reescrita el 2026-09-12 por ADR-028.** Decía «ningún AUX marcado como monitor recibe writes»; el usuario autorizó el ajuste de monitores —«*Sí, y también para el ajuste normal de monitores*»— y se abrió cuando la ley del envío quedó medida, porque INV-004 exige un límite en decibeles y sin ley no hay cómo declararlo. **Lo que recibe escritura es sólo el nivel**, `i.N.aux.M.value`: 240 rutas, veinticuatro canales por diez auxiliares. **No** `mute`, `pan`, `post` ni `postproc`, aunque el clasificador las ponga bajo el mismo tipo —`post` y `postproc` deciden si el envío se deriva antes o después del procesamiento, o sea que escribirlas recablea el monitor en vez de ajustarlo (medido en el ítem 95)—. **No** el fader del bus, `a.N.mix`, que es el volumen entero de esa cuña. Y sigue la excepción anterior: sends de Player L/R hacia AUX de monitor, únicamente hacia −∞ y solo dentro de PLAYER_RESERVE/restauración. Tres guardas en el motor: sólo el nivel, en **forma canónica y dentro del rango real** (`esNivelDeEnvioAMonitor`, no una expresión regular: con `\d+` sin cota pasaban `i.24.aux.0.value` y, peor, el alias `i.03.aux.1.value` esquivaba el techo puesto en `i.3.aux.1.value`, porque el estado por ruta se indexa por la cadena cruda); **techo por ruta sólo si la aplicación bajó ese envío**; y nada durante el show. **Y sobre el techo hay que decir dos cosas que esta fila decía mal.** Describía el ancla *derogada* —«el valor que tenía cuando el asistente la tocó por primera vez»—, que es la que rompía el caso principal: con todo abajo al empezar el soundcheck, la aplicación no podía subir nada. El usuario la hizo cambiar eligiendo «**Sólo si la app bajó**». Y la firmaba como «palabras del usuario», que es el error exacto que ADR-028 corrigió en sí mismo: **el anclaje es la operacionalización del agente**. El usuario dijo «hasta donde estaba antes de que *yo* lo bajara»; lo implementado se ancla en lo que bajó **la aplicación**, y la diferencia no se tapa (está en `packages/safety/src/types.ts`). **Y desde el 2026-09-17 hay una cuarta guarda, el techo de nominal.** ADR-034 separó **poner el nivel** de una cuña de **retocarla**: con las cuñas en el piso al empezar el soundcheck, el presupuesto de 4 dB por sesión hacía imposible el paso 3 del soundcheck del usuario —subir el auxiliar para que el músico tenga referencia son más de veinte decibeles—. Mientras la ruta **no tiene nivel establecido**, el presupuesto acumulado se suspende y lo reemplaza un **techo absoluto en 0 dB, nominal** (`Limite.techoAbsoluto`), decisión del usuario entre cuatro opciones; siguen rigiendo los 2 dB por paso y la exigencia de una medición entre un paso y el siguiente, que es lo que hace de esto una rampa y no una corrida. **El techo rige también al retocar**, segunda decisión del usuario del mismo día entre tres opciones: es un tope del parámetro y no de la operación, porque si no un nivel establecido apenas debajo de nominal se cruzaba con un retoque normal. Establecido el nivel, vuelve el presupuesto de ADR-028 **medido desde el ancla**, que `historialDeLaSesion` reconstruye de `EntradaDiario.nivelEstablecidoEn`. **No lo declara quien propone, y se cruza**: una etiqueta que suspende un presupuesto y que la pone quien quiere el permiso es pedir la exención diciendo que se la merece. **Esta frase estuvo escrita antes de ser cierta.** La primera versión aceptaba la lista del diario tal cual, y una auditoría adversarial del 2026-09-17 movió **30 dB de ganancia de previo** con el motor viendo el acumulado en cero. Hoy se exigen tres cosas: que esa transacción haya movido la ruta **y que el cambio se haya verificado**; que el parámetro **declare techo**; y que la ruta **no estuviera ya establecida** —volver a marcarla devolvía el presupuesto entero, sin límite—. **Y lo que falta decir, porque afloja:** quien *marca* el nivel como establecido es la pantalla de monitor, que no existe todavía, así que **hoy ninguna ruta llega a establecerse y el envío a monitor vive permanentemente en la primera operación**, sin presupuesto acumulado y acotado por el techo. **Corregido en esta misma fila, que mentía:** decía que «el techo todavía no protege en la aplicación porque el historial de sesión no existe». El historial existe desde el 2026-09-17 y llega a los dos servicios; `techoPorRuta` lo llena `BajarEnvioService` con `registrarTecho`. | Estático: paths escribibles ∩ paths de AUX monitor = los 240 `i.N.aux.M.value` más {`p.0.aux.*.value`, `p.1.aux.*.value`}; la cuenta total de rutas escribibles pasó de 690 a **930**, exactamente +240, con un test que lo comprueba por familia y no sólo por total. **Los envíos del reproductor están en el enunciado y hoy el test los verifica vacíos**: se rechazan por `SIN_LIMITE_DECLARADO`, porque `PLAYER_SEND` no tiene entrada en `LIMITES`. La excepción está enunciada y no implementada, y hasta que lo esté esta fila describe dos cosas distintas. | MVP1, ADR-028 |
