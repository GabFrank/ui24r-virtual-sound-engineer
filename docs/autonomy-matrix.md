...
**INV-010, corregida el 2026-09-12.** Esto decía «ningún envío de monitor recibe escrituras, en ningún nivel de autonomía y por ninguna versión». **Reescrita por ADR-028 el 2026-09-12.** Ya no es «nunca»: el usuario autorizó el ajuste del nivel de monitores y se abrieron las 240 rutas `i.N.aux.M.value`, con techo y sin show. Lo que sigue cerrado es todo lo demás del envío --`mute`, `pan`, `post`, `postproc`-- y el fader del bus. Lo que **sí** sigue valiendo es el espíritu de la frase, y hay que separarlo de la letra: un **automatizador** que altera lo que oye un músico en el escenario sin que nadie lo pida no es un producto mejor. Por eso el nivel se ajusta en soundcheck, con techo, y el rechazo por `sessionState === 'SHOW'` está en el motor.
