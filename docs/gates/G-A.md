...
| P0.9 | Avalancha detectada | bloqueante | 10 de 10 | **10 de 10 contra la consola el 2026-09-10** con una ráfaga de 16 rutas, y además con un `LOADSNAPSHOT` de verdad, que también dispara el aviso. Ya no depende del simulador. Pero el recall destapó un defecto: la causa sale `DESCONOCIDA` y nunca `SNAPSHOT_RECALL`, porque `var.currentSnapshot` llega como `SETS` y el almacén confirmado solo procesa `SETD`. **La mitad de INV-021 que habla de la instantánea es código inalcanzable**, así que un recall de menos de diez rutas no invalida nada. Y el aviso informa 10 cuando cambiaron 45 | 🟡 |
