...
| P0.2a | Especificación del protocolo versión 1 generada | bloqueante | sí | **Sí, y estaba desde el 2026-09-08 sin marcar acá.** `docs/protocol-spec.md`, versión 1; `docs/capability-matrix.md`, 77 filas con estado, versión 1 | ✅ |
...
| P0.9 | Avalancha detectada | bloqueante | 10 de 10 | **Cerrado el 2026-09-10, por los dos caminos.** Una ráfaga de 16 rutas: 10 de 10. Y diez `LOADSNAPSHOT` reales: **10 de 10, con causa `SNAPSHOT_RECALL` las diez veces**, once instantáneas creadas y borradas, cero claves distintas al terminar. Ya no depende del simulador, que disparaba la avalancha porque nosotros se lo pedíamos. Cerrarlo exigió arreglar el defecto que destapó: el puntero de instantánea llega como `SETS` y el almacén confirmado solo procesaba `SETD`, así que la mitad de INV-021 que habla del recall era código inalcanzable | ✅ |
...
| P0.8 | Alcance de la recuperación documentado | bloqueante | campo por campo | **Contestado el 2026-09-10: 44 de 45 campos vuelven.** Las seis familias que el criterio nombra, medidas con un `LOADSNAPSHOT` real: ganancia, fantasma, patcheo, retardos y reproductor vuelven; **la supresión de realimentación no**. `m.afs.enabled` es el único de los 45 que el recall no devuelve, y no es un campo cualquiera: el supresor le mete filtros de −18 dB al audio por su cuenta. Cero efectos colaterales sobre 6.700 claves | 🟡 |
