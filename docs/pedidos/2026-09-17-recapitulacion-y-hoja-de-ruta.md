...
| 1 | **Pantalla de monitor**. **Al 2026-09-20 se elige al músico, se ve todo lo que le llega a su cuña --los 32 caminos, con su instrumento primero-- y se la sube**: la pantalla encadena subir, escuchar y anotar, un envío por vez, con la cuenta regresiva y el botón de cancelar. Le falta **sólo el acto de marcar «así está bien»**, que no existe en ninguna capa: hasta que exista, toda cuña vive en la primera operación de ADR-034.  El techo desde silencio **ya estaba decidido** (ADR-028: si la app no bajó, no hay techo). Lo que apareció al empezar: el asistente sólo sabe bajar, el presupuesto de 4 dB por sesión hace imposible la rampa, y el silencio queda fuera del tramo medido. Decidido en [ADR-034](../adr/ADR-034-poner-el-nivel-de-monitor-y-retocarlo.md) | 9, 3 | no |
