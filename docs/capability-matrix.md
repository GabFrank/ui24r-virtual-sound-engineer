...
**Versión 1.** Derivada de la auditoría técnica contra `soundcraft-ui-connection` v7.0.3, y **con las filas marcadas en la columna *Probado* verificadas contra una consola real el 2026-09-08** (sesiones locales con hardware, SPK-P0.1, SPK-P0.2a y la parte de SPK-P0.10b que se midió con tonos generados en la máquina de desarrollo, que es la forma de la escala pero no su correspondencia con dBFS absolutos). El resto sigue sin probar.
...
| Medidores: reparto de bytes y escala | `vuProcessor.*` | `VU2` | CONFIRMADO, y **medido con una fuente de nivel conocido** el 2026-09-08: la escala **es lineal en decibeles** —desvío máximo de 0,21 dB en el barrido más fino— y su recorrido es de **~84,5 dB (±0,6)**, no los 80 que dice `VU_RANGE` en el `mixer.html`. Queda `dB = 84,5 · posición − 84,5`, **0,352 dB por escalón del byte**. Balística y tasa DESCONOCIDAS, y la correspondencia con **dBFS absolutos sin medir**: la ganancia analógica del camino era desconocida, así que valen las diferencias y no los valores absolutos | ✅ | P0.2a, P0.10b |
...
| Difusión de una escritura a los demás clientes | — | — | **CONFIRMADO.** El cambio escrito por un cliente sí llega a los otros: la aplicación en la tablet lo registró como `cambio_externo` en el mismo instante. **Sin medir**: si la difusión alcanza a una segunda conexión del mismo aparato, y con qué latencia | ✅ | P0.1, ACK-POLICY |
