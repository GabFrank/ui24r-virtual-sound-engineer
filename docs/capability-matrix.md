...
| Crear el punto de retorno antes de escribir | — | `CREATESHOW`, `SAVESNAPSHOT`, `SNAPSHOTLIST` | **MEDIDO y funcionando el 2026-09-09.** La aplicación guarda en su **propio show `VSE`** —nunca en los del usuario, y el show no es un parámetro del comando— con nombre `VSE_AUTO_<ms>`, y lo **verifica releyendo la lista** (INV-001). **No sabe borrar**: el protocolo tiene `DELETESNAPSHOT` y el módulo no lo construye. **Efecto secundario medido**: guardar cambia `var.currentSnapshot` | ✅ | P0.1 |
